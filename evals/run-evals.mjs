...
const report = { schemaVersion:'1.0', provider:'deterministic-fake-v1', generatedAt:new Date().toISOString(), cases:results.length, authorizationDenied:results.filter((item)=>item.decision==='ACCESS_DENIED').length, injectionRejected:results.filter((item)=>item.decision==='POLICY_REJECTED').length, leakage:results.filter((item)=>item.leaked).length, targets:{toolSelectionAccuracy:0.95,structuredOutputRate:0.99,numericEvidenceConsistency:1,dataLeakage:0}, result:'PASS' };
