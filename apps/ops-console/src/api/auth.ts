...
export async function beginLogin(): Promise<void> {
  ...
  window.location.assign(loginUrl(challenge, state));
}
...
export async function completeLogin(callbackUrl = window.location.href): Promise<void> {
  ...
  const session = useSessionStore();
  session.establish(tokens.access_token, displaySubject(tokens.access_token), tokens.id_token);
  if (sessionExpiryTimer) window.clearTimeout(sessionExpiryTimer);
  sessionExpiryTimer = window.setTimeout(clearLocalSession, Math.max(1, (tokens.expires_in ?? 300) - 5) * 1_000);
}

export function loginUrl(challenge: string, state: string): string {
  const url = new URL(`${issuer}/protocol/openid-connect/auth`);
  url.search = new URLSearchParams({ response_type: 'code', client_id: clientId, redirect_uri: loginRedirectUri(), scope: scopes, state, code_challenge: challenge, code_challenge_method: 'S256', prompt: 'login' }).toString();
  return url.toString();
}

export function logoutUrl(idToken?: string): string {
  const url = new URL(`${issuer}/protocol/openid-connect/logout`);
  const parameters = new URLSearchParams({ client_id: clientId, post_logout_redirect_uri: new URL('/', window.location.origin).toString() });
  if (idToken) parameters.set('id_token_hint', idToken);
  url.search = parameters.toString();
  return url.toString();
}

function loginRedirectUri(): string { return new URL('/auth/callback', window.location.origin).toString(); }
